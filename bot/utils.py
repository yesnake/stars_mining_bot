...
class ErrorHandler:
    ...
    async def handle_user_error(
        ...
        logger.error(f"Error during {context} for user {user_id}: {error}", exc_info=True)
        try:
...
@asynccontextmanager
async def safe_session(sessionmaker: async_sessionmaker) -> AsyncGenerator[AsyncSession, None]:
